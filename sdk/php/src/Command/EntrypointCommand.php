    ...
    private function getTypeDef(ListOfType|Type $type): TypeDef
    {
        ...
        switch ($type->typeDefKind) {
            ...
            case TypeDefKind::SCALAR_KIND:
                return $typeDef->withScalar(NormalizesClassName::shorten(
                    $type->name,
                ));
            case TypeDefKind::ENUM_KIND:
                return $typeDef->withEnum(NormalizesClassName::shorten(
                    $type->name,
                ));
            ...
            case TypeDefKind::OBJECT_KIND:
                if ($type->isIdable()) {
                    return $typeDef->withObject(NormalizesClassName::shorten(
                        $type->name,
                    ));
                }
